...
class AuthContainer extends _react.Component {
  constructor(props) {
    super(props);
  ...
  async doLogin(userObject) {
    ...
    if (errorObject) {
      ...
      try {
        const response = await _jsLib.Http.post(apiUrl, userObject);
  ...
  async doSocialLogin(userObject, network) {
    ...
    if (errorObject) {
      ...
      try {
        const response = await _jsLib.Http.post(apiUrl, {
          network,
          ...userObject
        });
  ...
  mountProps() {
    ...
    authProps.changeRoute = this.changeRoute; // Specific props for each view
  ...
  render() {
    return _react.default.cloneElement(this.getView(), this.mountProps());
  }
